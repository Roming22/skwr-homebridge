...
path = [ key for key in sys.argv[-1].split(".") if key ]
for key in path:
  data = data[key]
print(data)
' $*
...
ACCESSORY_FILES=$(sudo docker exec "$CONTAINER" find persist/ -name 'AccessoryInfo.*.json' | grep -v ${MAC//:/}) || true
...
ACCESSORY_FILES=($ACCESSORY_FILES) || true
# Restore IFS
IFS=$SAVEIFS || true
...
do
  ...
  PORT=$(sudo docker logs "$CONTAINER" | egrep "Homebridge v[0-9.]+ is running on port" | tail -n 1 | awk -F ' ' '{print $NF}' | cut -d. -f1)

  create_service_file "$NAME" $CATEGORY "$MAC" "$PORT" "$SETUPID"

done || true

echo "Devices registered to avahi-daemon"